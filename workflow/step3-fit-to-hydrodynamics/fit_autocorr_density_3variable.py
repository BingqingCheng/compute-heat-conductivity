...
def main(stemperature=1.0, sdensity=0.2, sysname="density-real"):
    ...
    results_allk = np.zeros((numk,10), dtype=float)
    ...
    for testindex in range(1,numk):
        ...
        def func(x, a, b, c):
            return C_k_t(x, ksqr_now, gamma_now , a/cp_now, c, b)
        try:
        ...
        results_allk[testindex, -6:] = [popt[0], perr[0], popt[1], perr[1], popt[2], perr[2]]

    results_no_nan = [ a for a in results_allk if a[-1]>0 ]
    np.savetxt("fit-autocorr-"+sysname+"-3variable.dat", results_no_nan ,header='# k_sqr kx ky kz lambda lambda_error b b_error cs cs_error')
